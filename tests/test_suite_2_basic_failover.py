...
from util_socket_methods import (
    resetActiveClient,
    checkPortOpen,
    measureSocketDowntime,
    SocketDowntimeTracker,
)
...
class TestBasicFailover:
    ...
    async def test_2_1_connection_persistence_to_client2(self):
        ...
        echoServer = UtilTcpSocket("echo_persistence_client2", shouldEchoData=True)
        ...
        try:
            ...
            from util_socket_methods import waitForCondition, waitForPortsOpenCallable
            logger.info(f"Waiting for client2 tunnel port {tunnelPort} to be available...")
            ...
            assert portAvailable, f"Client2 tunnel port {tunnelPort} not available within timeout"
            ...
            try:
                ...
                for i in range(30):
                    ...
                    receivedData, success = await conn.sendDataExpectEcho(sentBytes, timeout=5.0)
                    if success and receivedData == sentBytes:
                        successCount += 1
                        logger.debug(f"Test 2.1: Message {i+1} successful")
                    else:
                        logger.warning(f"Test 2.1: Message {i+1} data mismatch")
                    
                    await asyncio.sleep(1.0)

                logger.info(f"Test 2.1: {successCount}/30 messages successful during failover")
                assert successCount == 30, f"Only {successCount}/30 messages succeeded during failover"
                logger.info("Test 2.1: Successfully maintained connection during failover to client 2")
            finally:
                await conn.close()
        finally:
    ...
    async def test_2_2_connection_persistence_to_client1(self):
        ...
        echoPort = portConfig.clientToServerTun1ConnectPort
        echoServer = UtilTcpSocket("echo_persistence_client1", shouldEchoData=True)
        listening = await echoServer.startListen(port=echoPort, host="0.0.0.0")
        assert listening, f"Failed to start echo server on port {echoPort}"
            ...
            try:
                ...
                for i in range(30):
                    ...
                    if success and receivedData == sentBytes:
                        ...
                        logger.warning(f"Test 2.2: Message {i+1} data mismatch")
                    
                    await asyncio.sleep(1.0)

                logger.info(f"Test 2.2: {successCount}/30 messages successful during failover")
                assert successCount == 30, f"Only {successCount}/30 messages succeeded during failover"
                logger.info("Test 2.2: Successfully maintained connection during failover to client 1")
            finally:
                await conn.close()
        finally:
            await echoServer.close()
    ...
    async def test_2_3_connection_persistence_to_client2_second(self):
        ...
        # Start echo server for client2 tunnel
        echoPort = portConfig.clientToServerTun1ConnectPort
        echoServer = UtilTcpSocket("echo_persistence_client2_2nd", shouldEchoData=True)
        listening = await echoServer.startListen(port=echoPort, host="0.0.0.0")
        assert listening, f"Failed to start echo server on port {echoPort}"
            ...
            try:
                ...
                for i in range(30):
                    ...
                    if success and receivedData == sentBytes:
                        ...
                        logger.warning(f"Test 2.3: Message {i+1} data mismatch")
                    
                    await asyncio.sleep(1.0)

                logger.info(f"Test 2.3: {successCount}/30 messages successful during failover")
                assert successCount == 30, f"Only {successCount}/30 messages succeeded during failover"
                logger.info("Test 2.3: Successfully maintained connection during failover to client 2 (second)")
            finally:
                await conn.close()
        finally:
            await echoServer.close()
    ...
    async def test_2_4_connection_persistence_to_client1_second(self):
        ...
        # Start echo server for client1 tunnel
        echoPort = portConfig.clientToServerTun1ConnectPort
        echoServer = UtilTcpSocket("echo_persistence_client1_2nd", shouldEchoData=True)
        listening = await echoServer.startListen(port=echoPort, host="0.0.0.0")
        assert listening, f"Failed to start echo server on port {echoPort}"
            ...
            try:
                ...
                for i in range(30):
                    ...
                    if success and receivedData == sentBytes:
                        ...
                        logger.warning(f"Test 2.4: Message {i+1} data mismatch")
                    
                    await asyncio.sleep(1.0)

                logger.info(f"Test 2.4: {successCount}/30 messages successful during failover")
                assert successCount == 30, f"Only {successCount}/30 messages succeeded during failover"
                logger.info("Test 2.4: Successfully maintained connection during failover to client 1 (second)")
            finally:
                await conn.close()
        finally:
            await echoServer.close()
    ...
    async def test_2_5_server_port_closure_timing_client2_failover(self):
        ...
        try:
            ...
            assert portsAvailable, f"Server ports {serverPorts} not available within 30 seconds"
            ...
            while (time.time() - startTime) < 5.0:  # Give 5 seconds max
                ...
                if len(closedPorts) == len(serverPorts):
                    ...
                    allClosed = True
                    logger.info(f"Test 2.5: All server ports closed after {closeTime:.2f}s")
                    assert closeTime <= 1.0, f"Server ports took {closeTime:.2f}s to close (expected ≤1s)"
                    break
            ...
            while (time.time() - startReopenTime) < (expectedTimeout + 5):
                ...
                if len(openPorts) == len(serverPorts):
                    ...
                    allReopened = True
                    logger.info(f"Test 2.5: All server ports reopened after {reopenTime:.2f}s (expected around {expectedTimeout}s)")
                    break
                
                await asyncio.sleep(0.5)

            assert allReopened, f"Server ports did not reopen within expected time range"
            logger.info("Test 2.5: Successfully validated server port closure and reopen timing during failover to client 2")
        ...
        try:
            ...
            while (time.time() - startTime) < 5.0:  # Give 5 seconds max
                ...
                if len(closedPorts) == len(serverPorts):
                    closeTime = time.time() - startTime
                    allClosed = True
                    logger.info(f"Test 2.6: All server ports closed after {closeTime:.2f}s")
                    assert closeTime <= 1.0, f"Server ports took {closeTime:.2f}s to close (expected ≤1s)"
                    break
            ...
            while (time.time() - startReopenTime) < (expectedTimeout + 5):
                ...
                if len(openPorts) == len(serverPorts):
                    reopenTime = time.time() - startReopenTime
                    allReopened = True
                    logger.info(f"Test 2.6: All server ports reopened after {reopenTime:.2f}s (expected around {expectedTimeout}s)")
                    break
                
                await asyncio.sleep(0.5)

            assert allReopened, f"Server ports did not reopen within expected time range"
            logger.info("Test 2.6: Successfully validated server port closure and reopen timing during failover to client 1")
    ...
    async def test_2_7_server_port_closure_timing_client2_second(self):
        ...
        # Start echo server for client2 tunnel
        echoPort = portConfig.clientToServerTun1ConnectPort
        echoServer = UtilTcpSocket("echo_timing_client2_2nd", shouldEchoData=True)
        listening = await echoServer.startListen(port=echoPort, host="0.0.0.0")
        assert listening, f"Failed to start echo server on port {echoPort}"
        ...
        try:
            ...
            while (time.time() - startTime) < 5.0:  # Give 5 seconds max
                ...
                if len(closedPorts) == len(serverPorts):
                    closeTime = time.time() - startTime
                    allClosed = True
                    logger.info(f"Test 2.7: All server ports closed after {closeTime:.2f}s")
                    assert closeTime <= 1.0, f"Server ports took {closeTime:.2f}s to close (expected ≤1s)"
                    break
            ...
            while (time.time() - startReopenTime) < (expectedTimeout + 5):
                ...
                if len(openPorts) == len(serverPorts):
                    reopenTime = time.time() - startReopenTime
                    allReopened = True
                    logger.info(f"Test 2.7: All server ports reopened after {reopenTime:.2f}s (expected around {expectedTimeout}s)")
                    break
                
                await asyncio.sleep(0.5)

            assert allReopened, f"Server ports did not reopen within expected time range"
            logger.info("Test 2.7: Successfully validated server port closure and reopen timing during failover to client 2 (second)")
    ...
    async def test_2_8_server_port_closure_timing_client1_second(self):
        ...
        # Start echo server for client1 tunnel
        echoPort = portConfig.clientToServerTun1ConnectPort
        echoServer = UtilTcpSocket("echo_timing_client1_2nd", shouldEchoData=True)
        listening = await echoServer.startListen(port=echoPort, host="0.0.0.0")
        assert listening, f"Failed to start echo server on port {echoPort}"
        ...
        try:
            ...
            while (time.time() - startTime) < 5.0:  # Give 5 seconds max
                ...
                if len(closedPorts) == len(serverPorts):
                    closeTime = time.time() - startTime
                    allClosed = True
                    logger.info(f"Test 2.8: All server ports closed after {closeTime:.2f}s")
                    assert closeTime <= 1.0, f"Server ports took {closeTime:.2f}s to close (expected ≤1s)"
                    break
            ...
            while (time.time() - startReopenTime) < (expectedTimeout + 5):
                ...
                if len(openPorts) == len(serverPorts):
                    reopenTime = time.time() - startReopenTime
                    allReopened = True
                    logger.info(f"Test 2.8: All server ports reopened after {reopenTime:.2f}s (expected around {expectedTimeout}s)")
                    break
                
                await asyncio.sleep(0.5)

            assert allReopened, f"Server ports did not reopen within expected time range"
            logger.info("Test 2.8: Successfully validated server port closure and reopen timing during failover to client 1 (second)")
    ...
    async def test_2_9_standby_port_closure_timing_client2_failover(self):
        ...
        # Start echo server for client2 tunnel
        echoPort = portConfig.clientToServerTun1ConnectPort
        echoServer = UtilTcpSocket("echo_standby_timing_client2", shouldEchoData=True)
        listening = await echoServer.startListen(port=echoPort, host="0.0.0.0")
        assert listening, f"Failed to start echo server on port {echoPort}"
        ...
        try:
            ...
            assert portsAvailable, f"Client1 ports {client1Ports} not available within 30 seconds"
            ...
            while (time.time() - startTime) < 5.0:  # Give 5 seconds max
                ...
                if len(closedPorts) == len(client1Ports):
                    closeTime = time.time() - startTime
                    allClosed = True
                    logger.info(f"Test 2.9: All client1 standby ports closed after {closeTime:.2f}s")
                    assert closeTime <= 1.0, f"Client1 standby ports took {closeTime:.2f}s to close (expected ≤1s)"
                    break
                
                await asyncio.sleep(0.1)

            assert allClosed, "Client1 standby ports did not close within expected time"
            ...
            while (time.time() - startReopenTime) < (expectedTimeout + 5):
                ...
                if len(openPorts) == len(client1Ports):
                    reopenTime = time.time() - startReopenTime
                    allReopened = True
                    logger.info(f"Test 2.9: All client1 standby ports reopened after {reopenTime:.2f}s (expected around {expectedTimeout}s)")
                    break
                
                await asyncio.sleep(0.5)

            assert allReopened, f"Client1 standby ports did not reopen within expected time range"
            logger.info("Test 2.9: Successfully validated standby port closure and reopen timing during failover to client 2")
    ...
    async def test_2_10_standby_port_closure_timing_client1_failover(self):
        ...
        # Start echo server for client1 tunnel
        echoPort = portConfig.clientToServerTun1ConnectPort
        echoServer = UtilTcpSocket("echo_standby_timing_client1", shouldEchoData=True)
        listening = await echoServer.startListen(port=echoPort, host="0.0.0.0")
        assert listening, f"Failed to start echo server on port {echoPort}"
        ...
        try:
            ...
            assert portsAvailable, f"Client2 ports {client2Ports} not available within 30 seconds"
            ...
            while (time.time() - startTime) < 5.0:  # Give 5 seconds max
                ...
                if len(closedPorts) == len(client2Ports):
                    closeTime = time.time() - startTime
                    allClosed = True
                    logger.info(f"Test 2.10: All client2 standby ports closed after {closeTime:.2f}s")
                    assert closeTime <= 1.0, f"Client2 standby ports took {closeTime:.2f}s to close (expected ≤1s)"
                    break
                
                await asyncio.sleep(0.1)

            assert allClosed, "Client2 standby ports did not close within expected time"
            ...
            while (time.time() - startReopenTime) < (expectedTimeout + 5):
                ...
                if len(openPorts) == len(client2Ports):
                    reopenTime = time.time() - startReopenTime
                    allReopened = True
                    logger.info(f"Test 2.10: All client2 standby ports reopened after {reopenTime:.2f}s (expected around {expectedTimeout}s)")
                    break
                
                await asyncio.sleep(0.5)

            assert allReopened, f"Client2 standby ports did not reopen within expected time range"
            logger.info("Test 2.10: Successfully validated standby port closure and reopen timing during failover to client 1")
    ...
    async def test_2_11_standby_port_closure_timing_client2_second(self):
        ...
        # Start echo server for client2 tunnel
        echoPort = portConfig.clientToServerTun1ConnectPort
        echoServer = UtilTcpSocket("echo_standby_timing_client2_2nd", shouldEchoData=True)
        listening = await echoServer.startListen(port=echoPort, host="0.0.0.0")
        assert listening, f"Failed to start echo server on port {echoPort}"
        ...
        try:
            ...
            while (time.time() - startTime) < 5.0:  # Give 5 seconds max
                ...
                if len(closedPorts) == len(client1Ports):
                    closeTime = time.time() - startTime
                    allClosed = True
                    logger.info(f"Test 2.11: All client1 standby ports closed after {closeTime:.2f}s")
                    assert closeTime <= 1.0, f"Client1 standby ports took {closeTime:.2f}s to close (expected ≤1s)"
                    break
            ...
            while (time.time() - startReopenTime) < (expectedTimeout + 5):
                ...
                if len(openPorts) == len(client1Ports):
                    reopenTime = time.time() - startReopenTime
                    allReopened = True
                    logger.info(f"Test 2.11: All client1 standby ports reopened after {reopenTime:.2f}s (expected around {expectedTimeout}s)")
                    break
                
                await asyncio.sleep(0.5)

            assert allReopened, f"Client1 standby ports did not reopen within expected time range"
            logger.info("Test 2.11: Successfully validated standby port closure and reopen timing during failover to client 2 (second)")
    ...
    async def test_2_12_standby_port_closure_timing_client1_second(self):
        ...
        # Start echo server for client1 tunnel
        echoPort = portConfig.clientToServerTun1ConnectPort
        echoServer = UtilTcpSocket("echo_standby_timing_client1_2nd", shouldEchoData=True)
        listening = await echoServer.startListen(port=echoPort, host="0.0.0.0")
        assert listening, f"Failed to start echo server on port {echoPort}"
        ...
        try:
            ...
            while (time.time() - startTime) < 5.0:  # Give 5 seconds max
                ...
                if len(closedPorts) == len(client2Ports):
                    closeTime = time.time() - startTime
                    allClosed = True
                    logger.info(f"Test 2.12: All client2 standby ports closed after {closeTime:.2f}s")
                    assert closeTime <= 1.0, f"Client2 standby ports took {closeTime:.2f}s to close (expected ≤1s)"
                    break
            ...
            while (time.time() - startReopenTime) < (expectedTimeout + 5):
                ...
                if len(openPorts) == len(client2Ports):
                    reopenTime = time.time() - startReopenTime
                    allReopened = True
                    logger.info(f"Test 2.12: All client2 standby ports reopened after {reopenTime:.2f}s (expected around {expectedTimeout}s)")
                    break
                
                await asyncio.sleep(0.5)

            assert allReopened, f"Client2 standby ports did not reopen within expected time range"
            logger.info("Test 2.12: Successfully validated standby port closure and reopen timing during failover to client 1 (second)")
    ...
    async def test_2_13_final_functional_validation(self):
        ...
        echoServerClient = UtilTcpSocket("echo_final_client", shouldEchoData=True)
        listeningClient = await echoServerClient.startListen(port=echoPortClient, host="0.0.0.0")
        assert listeningClient, f"Failed to start client echo server on port {echoPortClient}"

        echoPortServer = portConfig.serverToClient1Tun1ConnectPort
        echoServerServer = UtilTcpSocket("echo_final_server", shouldEchoData=True)
        listeningServer = await echoServerServer.startListen(port=echoPortServer, host="0.0.0.0")
        assert listeningServer, f"Failed to start server echo server on port {echoPortServer}"
        ...
        try:
            ...
            receivedData, success = await clientConn.sendDataExpectEcho(testData)
            await clientConn.close()

            assert success, "Failed to send/receive on client-to-server tunnel"
            assert receivedData == testData, "Data mismatch on client-to-server tunnel"
            ...
            receivedData, success = await serverConn.sendDataExpectEcho(testData)
            await serverConn.close()

            assert success, "Failed to send/receive on server-to-client tunnel"
            assert receivedData == testData, "Data mismatch on server-to-client tunnel"

            logger.info("Test 2.13: Successfully completed final functional validation")
